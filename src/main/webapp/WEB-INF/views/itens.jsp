			...
			<tbody>
				<c:forEach items="${carrinhoCompras.itens}" var="item">
					<tr>
						...
						<td class="item-title">${item.key.produto.titulo}-${item.key.tipoPreco}</td>
						<td class="numeric-cell" ${item.key.preco}></td>
						<td class="quantity-input-cell"><input type="number" min="1" id="quantidade" name="quantidade"
							value="${item.value }" /></td>
						<td class="numeric-cell">${carrinhoCompras.getTotal(item.key)}</td>
						<td class="remove-item">
							<form action="${s:mvcUrl('CCC#remove').arg(0,item.key.produto.id).arg(1,item.key.tipoPreco).build()}"
								method="post">
								<!--  input type="hidden" id="produto" name="produto" value="${item.key.produto.id}"/-->
								<input type="image"
